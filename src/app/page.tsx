  ...
  useEffect(() => {
    async function fetchMinPrice() {
      try {
        ...
        if (plans.length > 0) {
          // Find the absolute minimum price across all plans and age tiers
          let min = Infinity;
          plans.forEach(plan => {
            if (plan.ageTiers && plan.ageTiers.length > 0) {
              plan.ageTiers.forEach((tier: any) => {
                if (tier.options) {
                  Object.values(tier.options).forEach((premium: any) => {
                    if (premium > 0 && premium < min) min = premium;
                  });
                }
              });
            } else if (plan.options) {
              // Backward compatibility for flat options
              plan.options.forEach((opt: any) => {
                const price = opt.premium || opt.basePrice;
